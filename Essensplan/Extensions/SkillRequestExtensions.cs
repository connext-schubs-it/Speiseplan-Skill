...
namespace AssistServer.Extension.NewFolder
{
   public static class SkillRequestExtensions
   {
      ...
      public static string GetSpokenText(this SkillRequest anfrage, string slotName, string defaultValue)
      {
         IntentRequest intentRequest = anfrage.Request as IntentRequest;
         Slot attribute = null;
         String text = null;
         if (intentRequest.Intent.Slots != null && intentRequest.Intent.Slots.TryGetValue(slotName, out attribute))
         {
            if (attribute != null)
            {
               if (attribute.Resolution != null)
               {
                  text = attribute.Resolution.Authorities[0].Values[0].Value.Id;
               }
               else
               {
